  ...
  if (rows.length === 0) {
    return (
      <div className="card flex items-center justify-between gap-4 px-5 py-5">
        <div>
          <h3 className="text-[15px] font-semibold text-ink">You haven't submitted a case yet.</h3>
          <p className="mt-1 text-[13px] text-ink-soft">
            Reports you submit on this device will appear here with their progress.
          </p>
        </div>
        <Link
          href="/report"
  ...
  return (
    <div className="space-y-3">
      ...
      {rows.length > 3 && (
        ...
        >
          View all your cases <Icon name="arrow-right" className="h-4 w-4" />
        </Link>
      )}
    </div>
  );
