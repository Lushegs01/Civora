...
interface Summary {
  ...
  sourcesUsed: string[];
  provider: string;
  /** False when the text came from Civora's built-in analyzer, not a model. */
  aiGenerated: boolean;
  generatedAt: string;
  disclaimer: string;
}
        ...
        <div className="border-t border-line/60 px-5 py-4">
          ...
          {summary && (
            <div className="space-y-4">
              ...
              <p className="flex gap-2 rounded-xl bg-muted px-3 py-2 text-[12px] leading-relaxed text-ink-soft">
                <Icon name={summary.aiGenerated ? "sparkles" : "info"} className="mt-0.5 h-3.5 w-3.5 shrink-0" />
                <span>
                  <strong className="font-semibold text-ink">
                    {summary.aiGenerated
                      ? t("case.ai.modelGenerated", locale) || "AI-assisted."
                      : t("case.ai.builtIn", locale) || "Generated by Civora's built-in analyzer."}
                  </strong>{" "}
                  {summary.disclaimer}
                </span>
              </p>
